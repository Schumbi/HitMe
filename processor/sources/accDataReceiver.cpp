...
uint32_t oldTime = 0;
void AccDataReceiver::processData (const QByteArray& data)
{
    conv32_t conv32;
    conv16_t conv16;
    // get time stamp
    ...
    emit dataReady (dataVec);

    oldTime = time;
}

AccDataReceiver::AccDataReceiver (QObject *parent) : QThread (parent)
{
...
void AccDataReceiver::readingPendingDatagrams()
{
    QByteArray datagram;
